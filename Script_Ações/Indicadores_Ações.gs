...
/**
 * MÓDULO AÇÕES: Captura e processamento em lotes controlados (16 Colunas: B até Q).
 */
function processarBlocoAcoesDinamico(aba) {
  ...
  const erros = [];
  const TOTAL_COLUNAS_MAPEADAS = 11; // Atualizado para 11 colunas (B até L)
  ...
  for (let i = 0; i < tickers.length; i += TAMANHO_LOTE) {
    ...
    responses.forEach((res, index) => {
      ...
      try {
        ...
        // Mapeamento dos Dados Brutos + Beta (11 colunas)
  ...
  // GRAVAÇÃO EM MASSA (Colunas B até Q)
